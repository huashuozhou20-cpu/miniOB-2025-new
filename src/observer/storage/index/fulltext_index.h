...
private:
  ...
  std::unordered_map<std::string, std::unordered_set<RID>> inverted_index_;
  
  // 文档索引: RID -> set of words (用于删除和更新)
  std::unordered_map<RID, std::unordered_set<std::string>> document_index_;
  ...
  // 文档长度（用于 BM25 计算）
  std::unordered_map<RID, size_t> document_lengths_;
