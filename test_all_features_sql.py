...
# -*- coding: utf-8 -*-
"""
MiniOB 全部功能 SQL 测试脚本
使用 SQL 语句逐一验证所有功能是否可用
"""
...
def test_feature(name, sock_path, test_steps):
    """测试一个功能"""
    print(f"\n{'='*70}")
    print(f"测试: {name}")
    print(f"{'='*70}")
...
def main():
    ...
    try:
        # 1. drop-table
        results["drop-table"] = test_feature("drop-table", sock_path, [
            ("创建表", "CREATE TABLE test_drop(id INT, name CHAR(10));", True),
            ("删除表", "DROP TABLE test_drop;", True),
            ("验证删除", "SELECT * FROM test_drop;", False),  # 应该失败
        ])
        
        # 2. update
        results["update"] = test_feature("update", sock_path, [
        ...
        # 3. date
        results["date"] = test_feature("date", sock_path, [
        ...
        # 4. expression
        results["expression"] = test_feature("expression", sock_path, [
            ("算数表达式", "SELECT 1 + 2 * 3 AS result;", True),
            ("复杂表达式", "SELECT 10 - 5 / 2 AS result;", True),
        ])
        
        # 5. function
        results["function"] = test_feature("function", sock_path, [
        ...
        # 6. join-tables
        results["join-tables"] = test_feature("join-tables", sock_path, [
        ...
        # 7. group-by
        results["group-by"] = test_feature("group-by", sock_path, [
        ...
        # 8. order-by
        results["order-by"] = test_feature("order-by", sock_path, [
        ...
        # 9. null
        results["null"] = test_feature("null", sock_path, [
        ...
        # 10. alias
        results["alias"] = test_feature("alias", sock_path, [
        ...
        # 11. multi-index
        results["multi-index"] = test_feature("multi-index", sock_path, [
        ...
        # 12. unique
        results["unique"] = test_feature("unique", sock_path, [
        ...
        # 13. text
        results["text"] = test_feature("text", sock_path, [
        ...
        # 14. union
        results["union"] = test_feature("union", sock_path, [
        ...
        # 15. vector-basic
        results["vector-basic"] = test_feature("vector-basic", sock_path, [
        ...
        # 16. alter
        results["alter"] = test_feature("alter", sock_path, [
        ...
        # 17. simple-sub-query
        results["simple-sub-query"] = test_feature("simple-sub-query", sock_path, [
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(f"- 通过: {passed}\n")
            f.write(f"- 失败: {failed}\n")
            f.write(f"- 成功率: {passed*100//len(results)}%\n")
