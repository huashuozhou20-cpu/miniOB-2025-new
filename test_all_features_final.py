...
def check_port_listening(host, port, timeout=30):
    """检查端口是否在监听"""
...
def run_sql(host, port, sql, timeout=10):
    ...
    try:
        ...
        # 检查是否有错误信息
        has_error = "ERROR" in stderr.upper() or "FAIL" in stderr.upper() or "Connection refused" in stderr or "failed" in stderr.lower()
...
def test_feature(name, host, port, test_steps):
    ...
    if errors:
        print(f"  ❌ {name} 测试失败 ({passed_steps}/{len(test_steps)} 步骤成功)")
        return False, errors
    else:
        print(f"  ✅ {name} 测试通过 ({passed_steps}/{len(test_steps)} 步骤成功)")
        return True, []
...
def main():
    ...
    if check_port_listening(SERVER_HOST, SERVER_PORT, timeout=30):
        print("✓ Observer 启动成功，端口已监听")
        time.sleep(2)  # 再等待一下确保完全就绪
    else:
        print("✗ Observer 启动失败 - 端口未监听")
