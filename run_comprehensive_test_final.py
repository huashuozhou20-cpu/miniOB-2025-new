...
def run_sql(sql, timeout=10):
    ...
    try:
        ...
        # 判断成功：returncode 为 0 或输出中没有 ERROR
        success = (
            proc.returncode == 0 or 
            ("ERROR" not in stdout.upper() and "ERROR" not in stderr.upper() and
             "failed" not in stdout.lower() and "failed" not in stderr.lower() and
...
def test(name, sqls, description=""):
    ...
    for i, sql in enumerate(sqls, 1):
        ...
        if not success:
            ...
            if stdout and "ERROR" in stdout.upper():
                print(f"    输出: {stdout[:300]}")
            return False
        else:
            print(f"    ✅ 成功")
            if stdout and len(stdout.strip()) > 0 and len(stdout) < 300:
                lines = stdout.strip().split('\n')
                # 只显示非空行
                output_lines = [l for l in lines if l.strip() and not l.startswith('Welcome')]
                if output_lines:
                    print(f"    输出: {output_lines[0][:100]}")
...
def main():
    ...
    if not connected:
        print("❌ Observer 启动超时")
        print("检查 observer 输出:")
        try:
            stdout, _ = observer.communicate(timeout=2)
            print(stdout[-500:])
        except:
            pass
        observer.kill()
    ...
    try:
        ...
        print(f"✅ 通过: {passed}")
        print(f"❌ 失败: {failed}")
        print(f"通过率: {passed*100//len(results)}%")
